...
selex_path <- '/contrib/Alberto.Rovellini/data/age_at_selex.csv' # selectivity patterns
...
selex <- read.csv(selex_path, header = T) # age at fishing selex
...
biomage_val <- biomage %>% 
  slice_tail(n = 1) %>% # use last xxx years
...
f_t1 <- biom_age_t1 %>% left_join(catch_t1) %>%
  mutate(exp_rate = catch_val/biomass,
  ...
  select(Code, this_f, f)

# bind all
f_frame <- f_t1 %>%
  mutate(biomass = biomage_val,
         catch = catch_val)
